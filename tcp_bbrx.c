...
/* LT policer + aggressive-probe gate (net.ipv4.tcp_bbrx_loss_thresh, %%).
...
static u64 bbrx_sock_get_sec(const struct tcp_sock *tp)
{
...
static void bbrx_apply_aggressive_probe(struct sock *sk, bool may_probe)
{
	...
	if (!may_probe)
		return;
	if (bbr->full_bw_reached || bbr->lt_use_bw) {
...
#if IS_ENABLED(CONFIG_SYSCTL)
static unsigned int bbrx_loss_thresh_percent_min = 0;
static unsigned int bbrx_loss_thresh_percent_max = 99;

static struct ctl_table bbrx_sysctl_table[] = {
	{
		.procname	= "tcp_bbrx_loss_thresh",
		.data		= &bbrx_loss_thresh_percent,
		.maxlen		= sizeof(bbrx_loss_thresh_percent),
		.mode		= 0644,
...
#if IS_ENABLED(CONFIG_SYSCTL)
	bbrx_sysctl_header = register_net_sysctl(&init_net, "net/ipv4",
						 bbrx_sysctl_table);
	if (IS_ERR(bbrx_sysctl_header)) {
		ret = PTR_ERR(bbrx_sysctl_header);
		bbrx_sysctl_header = NULL;
		return ret;
	}
#endif
...
#endif
#if IS_ENABLED(CONFIG_SYSCTL)
	if (bbrx_sysctl_header) {
		unregister_net_sysctl_table(bbrx_sysctl_header);
		bbrx_sysctl_header = NULL;
	}
#endif
	return ret;
